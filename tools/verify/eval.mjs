...
const chrome = spawn("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", ["--headless=new", `--remote-debugging-port=${port}`, "--no-first-run", "--hide-scrollbars", "--disable-gpu", `--user-data-dir=/tmp/kh-eval-${port}`, `--window-size=${w},${h}`, "about:blank"], { stdio: "ignore" });
